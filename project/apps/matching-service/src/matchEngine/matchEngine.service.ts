...
import { MatchCriteriaDto, MatchDataDto, MatchRequestMsgDto } from '@repo/dtos/match';
...
export class MatchEngineService {
  ...
  async generateMatch(matchRequest: MatchRequestMsgDto) {
    ...
    const matchedData = await this.matchRedis.findPotentialMatch({category, complexity});

    if (matchedData) {
      this.logger.log(`Match found for user ${userId} and ${matchedData.userId}`);

      // Find the overlapping categories between both users
      const overlappingCategories = category.filter((value) => matchedData.category.includes(value));
